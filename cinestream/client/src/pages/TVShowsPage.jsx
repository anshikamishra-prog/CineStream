...
const TVShowsPage = () => {
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery({
    queryKey: ['tv', category, genreId, sortBy, page],
    queryFn:  () => mediaApi.getTVShows({ category, page, genre: genreId }),
    select:   (res) => res.data,
  ...
  const activeGenreName = genreId
    ? (genresData?.tvGenres || []).find((g) => String(g.id) === genreId)?.name
    : null;
  ...
  return (
    <div className="min-h-screen pt-24 pb-20">
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-12">
        ...
        {isError ? (
          <ErrorMessage title="Failed to Load TV Shows" onRetry={refetch} />
        ) : (
          <AnimatePresence mode="wait">
            ...
            >
              <MediaGrid items={shows} isLoading={isLoading || isFetching} />
              <Pagination
                page={page}
